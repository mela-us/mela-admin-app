...
import {
  Tabs as PopoverTabs,
  TabsContent as PopoverTabsContent,
  TabsList as PopoverTabsList,
  TabsTrigger as PopoverTabsTrigger,
...
export function MathEditor({ value, onChange, uploadedImages, setUploadedImages, placeholder = "Nhập nội dung...", height = "h-32" }: MathEditorProps) {
  ...
  useEffect(() => {
    ...
    html = html.replace(/<latex>(.*?)<\/latex>/g, (match, latex) => {
      try {
        ...
        return `<div class="inline-block px-2 py-1 bg-red-100 text-red-800 rounded font-mono">Error: ${error}</div>`
      }
    })
  ...
  const uploadImage = async (image: UploadedImage) => {
    setUploadedImages((prev) =>
      prev.map((img) => (img.id === image.id ? { ...img, status: "uploading", progress: 10 } : img)),
    )

    try {
      await new Promise((resolve) => setTimeout(resolve, 500))
      setUploadedImages((prev) => prev.map((img) => (img.id === image.id ? { ...img, progress: 30 } : img)))

      const response = await fetch("/api/upload/exercise-image", { method: "POST" });
      ...
      console.log("Upload image with presignedUrl: " + presignedUrl);

      // const mockResponse = {
      //   presignedUrl: "https://mock-presigned-url.example.com",
      //   imageUrl: `https://mela-storage-dev.s3.ap-southeast-1.amazonaws.com/exercises/images/${image.file.name.replace(/\s+/g, "_")}`,
      // }

      await new Promise((resolve) => setTimeout(resolve, 300))
      setUploadedImages((prev) => prev.map((img) => (img.id === image.id ? { ...img, progress: 70 } : img)))

      await new Promise((resolve) => setTimeout(resolve, 200))
      setUploadedImages((prev) =>
        prev.map((img) =>
          img.id === image.id ? { ...img, status: "success", progress: 100, url: imageUrl } : img,
        ),
      )
      ...
      setUploadedImages((prev) =>
        prev.map((img) => (img.id === image.id ? { ...img, status: "error", error: "Lỗi khi tải lên hình ảnh" } : img)),
      )
    }
  }
  ...
  const insertBlobImageTag = (image: UploadedImage) => {
    if (!image.blobUrl) return
    insertText(`<img src='${image.blobUrl}'>`)
  }
    ...
    <div className="border rounded-md shadow-sm">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="flex items-center space-x-1">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <PopoverTrigger asChild>
                          <Button variant="ghost" size="icon" className={`h-9 w-9 ${isLatexPopoverOpen ? "bg-red-500/50 hover:bg-red-500/80" : "hover:bg-gray-200"}`}
                            onClick={() => {
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Chọn hoặc nhập trực tiếp công thức
                          </p>
                        </div>
                        <Button
                          variant="ghost"
                ...
                <Popover open={isImagePopoverOpen}>
                  ...
                  <PopoverContent className="w-[350px] p-0" align="start" sideOffset={5}>
                    <PopoverTabs defaultValue="upload" className="w-full">
                      <div className="p-3 border-b">
                        <div className="flex items-start justify-between">
                          <div>
                            <h4 className="font-medium text-sm">Chèn hình ảnh</h4>
                            <p className="text-xs text-muted-foreground mt-1">Tải lên hoặc chọn hình ảnh đã tải</p>
                          </div>
                          <Button
                            variant="ghost"
                            size="icon"
                        ...
                        <PopoverTabsList className="mt-2 grid w-full grid-cols-2">
                          <PopoverTabsTrigger value="upload">Tải lên</PopoverTabsTrigger>
                          <PopoverTabsTrigger value="gallery">Kho ảnh ({uploadedImages.length})</PopoverTabsTrigger>
                        </PopoverTabsList>
                      </div>

                      <PopoverTabsContent value="upload" className="p-3 space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground text-center">
                          Hỗ trợ các định dạng: JPG, PNG, GIF, SVG
                        </p>
                      </PopoverTabsContent>

                      <PopoverTabsContent value="gallery" className="p-0">
                        <div onWheel={(e) => {
                          e.stopPropagation();
                            ...
                            <div className="grid grid-cols-2 gap-2 p-3">
                              {uploadedImages.map((image) => (
                                ...
                                >
                                  ...
                                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1">
                                    {image.status === "pending" && (
                                      <Button
                                        ...
                                        className="w-20 h-7 text-xs bg-blue-500 hover:bg-blue-600 gap-1"
                                      >
                                    ...
                                    {(image.status === "success" || image.status === "pending") && (
                                      <Button
                                        size="sm"
                                        variant="secondary"
                                        onClick={() => {
                                          image.status === "success"
                                            ? insertImageTag(image.url!)
                                            : insertBlobImageTag(image);
                                        }}
                                        className="w-20 h-7 text-xs gap-1"
                                      >
                                  ...
                                  {image.status === "error" && (
                                    <div className="absolute top-1 right-1">
                                      <Badge className="bg-red-500 text-white h-5 px-1.5">
                                        <X className="h-3 w-3" />
                                      </Badge>
                                    </div>
                                  )}
        ...
        <TabsContent value="preview" className="p-0 m-0">
          <div
            ref={previewRef}
            className={`w-full ${height} p-3 overflow-auto bg-gray-50 [&_img]:max-w-[300px] [&_img]:max-h-[300px] [&_img]:object-contain [&_img]:inline-block [&_img]:align-bottom `} style={{
              whiteSpace: 'pre-wrap',
              wordWrap: 'break-word',

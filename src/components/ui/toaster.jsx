import { ToastProvider as RadixToastProvider, Toast, ToastDescription, ToastTitle, ToastViewport } from './toast';
...
export function Toaster() {
  ...
  return (
    <RadixToastProvider swipeDirection="right" duration={2300}>
      {toasts.map(({ id, title, description, action, variant, ...props }) => (
        ...
        >
          <div className="flex-1 grid gap-0.5">
            ...
            {description && (
              <ToastDescription className="text-xs text-zinc-500 dark:text-zinc-400">{description}</ToastDescription>
            )}
          </div>
          {action}
        </Toast>

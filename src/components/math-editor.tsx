...
export default function MathEditor({
  ...
  const [tempImages, setTempImages] = useState<{ id: string; file: File; url: string }[]>([])
  ...
  const generatePreviewHtml = (text: string) => {
    ...
    const html = text.replace(/<latex>(.*?)<\/latex>/g, (match, latex) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setTempImages([...tempImages, { id: imageId, file, url: imageUrl }])
